...
static void update_info(void)
{
	...
	for (fw = 0, i = filecnt; i > 0; fw++, i /= 10)
		;
	mark = files[fileidx].flags & FF_MARK ? "🌻 " : ""; // symbol used for a mark
	l->p = l->buf;
	r->p = r->buf;
	if (mode == MODE_THUMB) {
    ...
    // END MARKCOUNT PATH PT 2
		if (img.ss.on) {
			if (img.ss.delay % 10 != 0)
				...
				bar_put(r, "%ds" BAR_SEP, img.ss.delay / 10); // if slideshow is on
		}
		if (img.gamma)
    ...
    bar_put(r, "[%0*d/%d]", fw, fileidx + 1, filecnt); // at the end
		if (info.f.err) {
      char* icon ="🌻 ";
      int icon_len = strlen(icon);
